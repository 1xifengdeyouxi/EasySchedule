...
//    这意味着：无论图标处于什么状态（正常、按下、聚焦等），都会显示colorOnBackground对应的颜色。
    fun createColorStateList(color: Int): ColorStateList {
        ...
        // 状态4：控件所在窗口“获得焦点”（应用在前台时）
        states[4] = intArrayOf(android.R.attr.state_window_focused)
        // 状态5：默认状态（无任何特殊状态）
        states[5] = intArrayOf()
        // 创建并返回ColorStateList：状态与颜色一一对应
//        它是一个 “状态 - 颜色” 映射表，当控件的状态（如按下、聚焦）发生变化时，会自动从列表中匹配对应的颜色并应用到图标上。
